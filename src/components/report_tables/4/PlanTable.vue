...
export default {
  computed:{
    ...mapGetters('plan',['getColumns1','getRows1','getDefaultItem1','getEditedIndex1','getEditedItem1'])
  },
  methods: {
    addRow() {
      this.$store.commit('plan/addRow1',{'editedIndex':this.getEditedIndex1,'editedItem':this.getEditedItem1})
      this.close()
      console.log(this.getRows1)
    },
    deleteItem(item) {
      confirm("Вы уверены что хотите удалить строку") && this.$store.commit('plan/deleteItem1',item)
    },
    editItem(item) {
      this.$store.commit('plan/editItem1',item)
      this.show_dialog = true;
    },
    close () {
      this.show_dialog = false
      setTimeout(() => {this.$store.commit('plan/setDefault1')}, 300)
    }
  },
  data() {
    return {
