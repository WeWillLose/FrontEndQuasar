<template>
    ...
    <div class="q-pa-sm q-gutter-sm">
      <q-table title="Проведение мероприятий (отделение, колледж)" :hide-no-data="true" :data="getRows1" :columns="getColumns1">
          ...
          <q-tr :props="props">
            ...
            <q-td key="score" :props="props">
              <q-input type="number" v-model="props.row.score"
                       :rules="[rules.withoutSpaces(),rules.numberBetweenNotStrictScore(0,1.5)]" dense autofocus></q-input>
            </q-td>
...
export default {
  computed:{
    ...mapGetters('report_tables/events',['getColumns1','getRows1','getDefaultItem1','getEditedIndex1','getEditedItem1'])
  },
  methods: {
    addRow() {
      this.$store.commit('report_tables/events/addRow1',{'editedIndex':this.getEditedIndex1,'editedItem':this.getEditedItem1})
      this.close()

    },
    deleteItem(item) {
      confirm("Вы уверены что хотите удалить строку") && this.$store.commit('report_tables/events/deleteItem1',item)
    },
    editItem(item) {
      this.$store.commit('report_tables/events/editItem1',item)
      this.show_dialog = true;
    },
    close () {
      this.show_dialog = false
      setTimeout(() => {this.$store.commit('report_tables/events/setDefault1')}, 300)
    }
  },
  data() {
    return {
