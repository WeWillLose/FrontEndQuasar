      ...
      >
        <template v-slot:before>
          <q-tabs
            ...
            class="text-teal"
          >
            <q-tab name="teacher" label="Учитель"/>
            <q-tab name="finish" label="Итог"/>
        ...
        <template v-slot:after>
          <q-card>
            <q-form @submit="onSubmit" ref="reportForm">
              <q-tab-panels
                v-model="tab"
                ref="reportTabs"
                keep-alive
              ...
              >
                ...
                <q-tab-panel name="finish">
                  <div class="column q-ma-md">
                    <div class="col">
                      <q-input style="max-width: 200px" v-model="sum1" readonly
                               label="Сумма баллов по 1-2 критерию:"></q-input>
                    </div>
                    <div class="col">
                      <q-input style="max-width: 200px" v-model="sum2" readonly
                               label="Сумма баллов по 3-10 критерию:"></q-input>
                    </div>
                    <div class="col">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      rules: () => rules,
      sum1() {
        return this.$store.getters["report_tables/comment/getScore"] + this.$store.getters["report_tables/creation/getScore"] +
          this.$store.getters["report_tables/proective/getScore"]
      },
      sum2() {
    ...
    methods: {
      validateReportForm(validationResult, refs) {
        validationResult.errCount = 0
        if ("firstName" in refs) {
          refs.firstName.validate()
          validationResult.firstNameErr = !!refs.firstName.hasError;
          validationResult.firstNameErr && validationResult.errCount++
        }
        if ("lastName" in refs) {
          refs.lastName.validate()
          validationResult.lastNameErr = !!refs.lastName.hasError;
          validationResult.lastNameErr && validationResult.errCount++
        }
        if ("patronymic" in refs) {
          refs.patronymic.validate()
          validationResult.patronymicErr = !!refs.patronymic.hasError;
          validationResult.patronymicErr && validationResult.errCount++
        }
        if ("year1" in refs) {
          refs.year1.validate()
          validationResult.year1Err = !!refs.year1.hasError;
          validationResult.year1Err && validationResult.errCount++
        }
        if ("year2" in refs) {
          refs.year2.validate()
          validationResult.year2Err = !!refs.year2.hasError;
          validationResult.year2Err && validationResult.errCount++
        }
        validationResult.errCount > 0 ? validationResult.formHasError = true : validationResult.formHasError = false
        return validationResult;

      },
      onSubmit() {
        let res = this.validateReportForm(this.validators, this.$refs)
        if (res.formHasError) {
          notifyApi.showErrorNotify("Невалидные данные")
          return;
        }
        let dict = {}
      ...
      async travel() {
        let traversal = this.names.reduce((promiseChain, item) => {
          return promiseChain.then(() => new Promise(resolve => {
              resolve()
              this.$refs.reportTabs.goTo(item)
            })
          )
        }, Promise.resolve())
        traversal.then(() => {
          this.$refs.reportTabs.goTo("teacher")
        })
      }
